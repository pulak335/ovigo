...
import style from "./ExploreSearchForm.module.css";
...
const ExploreSearchForm = ({
  ...
  const [destinationToggleIcon, setDestinationToggleIcon] = useState(false);
  ...
  //search value inner the state : district, division, location, destinationType

  //* location search api
  useEffect(() => {
    if (searchTerm) {
      fetch(`http://159.223.78.171:5000/searchSuggetions?term=${searchTerm}`)
  ...
  //*destination type api calling
  useEffect(() => {
    fetch("http://159.223.78.171:5000/searchSuggetions/destinationType")
      .then((res) => res.json())
      .then((data) => {
        setDestinationType(data?.data);
      });
  }, []);

  const handleDestinationToggle = (e) => {
    setDestination(e.target.textContent);
    setSearchValue({ ...searchValue, activity: e.target.textContent });
    setDestinationToggleIcon(false);
  };
...
export default ExploreSearchForm;
